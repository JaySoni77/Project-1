...
# Define the list to store users from stockholm
users_in_stockholm = []
...
with open('users.csv', 'r', encoding='utf-8') as file:
    ...
    for row in reader:
        ...
        # Check if the user is from stockholm
        if 'stockholm' in location:
            users_in_stockholm.append({
                'login': row['login'],
                'followers': int(row['followers'])
            })

# Sort users based on followers in descending order
top_users = sorted(users_in_stockholm, key=lambda x: x['followers'], reverse=True)
...
sorted_users = sorted(users_in_stockholm, key=lambda x: x['created_at'])
...
# Lists to store the followers and public repos of users from stockholm
followers = []
public_repos = []
